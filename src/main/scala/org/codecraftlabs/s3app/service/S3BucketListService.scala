...
import org.codecraftlabs.s3app.util.AwsRegionUtil
...
object S3BucketListService {
  ...
  def list(region: AwsRegion.Value): Option[List[S3Bucket]] = {
    logger.info("Listing all buckets")
    val s3Client = S3Client.builder().region(AwsRegionUtil.getRegion(region)).build()
